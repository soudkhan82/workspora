...
const tableMap: Record<string, string> = {
  ...
  project: "invoice_projects",
  projects: "invoice_projects",

  milestone: "invoice_milestones",
...
function getTable(body: any) {
  const type = String(
    body?.type || body?.dropdownType || body?.category || body?.kind || "",
  )
    .trim()
    .toLowerCase();
...
export async function GET() {
  try {
    const [clients, projects, milestones, statuses] = await Promise.all([
      readTable("invoice_clients"),
      readTable("invoice_projects"),
      readTable("invoice_milestones"),
      readTable("invoice_statuses"),
    ]);
